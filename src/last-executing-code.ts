...
function transformStatements(
  ...
  for (const statement of node.statements) {
    ...
    if (ranges && ranges.some((r) => sourceFile.text.substring(r.pos, r.end).includes('last-executing-code:disable'))) {
      statements.push(statement)
    ...
    statements.push(
      ts.createExpressionStatement(ts.createBinary(
        ...
        ts.createStringLiteral(`${path.relative(process.cwd(), sourceFile.fileName)}:${line + 1}:${character + 1}`),
      )),
      statement,
    )
  }
